...
function create_system_group() {
    ...
    if ! grep -q "^$SYSTEM_GROUP:" /etc/group; then
        ...
        echo -e "ℹ️  Group $SYSTEM_GROUP already exists."
    fi
...
function create_user_and_assign_group() {
    ...
    if ! id -u "$USERNAME" >/dev/null 2>&1; then
        ...
        echo -e "ℹ️  User $USERNAME already exists. Adding to group $SYSTEM_GROUP."
    ...
    if ! grep -q "$USERNAME ALL=(ALL) NOPASSWD: ALL" /etc/sudoers; then
        ...
        echo -e "ℹ️  User $USERNAME already has passwordless sudo privileges."
    fi
}
...
function setup_ssh_keys() {
    ...
    if [[ ! -f "$SSH_DIR/id_rsa" ]]; then
        ...
        echo -e "ℹ️  SSH keys already exist for $USERNAME."
    fi
    ...
    if [[ ! -f "$SSH_DIR/authorized_keys" ]]; then
        ...
        echo -e "ℹ️  authorized_keys already exists for $USERNAME."
    fi
}
...
read -p "➡️  (y/n): " create_ansible_user
if [[ $create_ansible_user == "y" || $create_ansible_user == "Y" ]]; then
    ...
    read -p "➡️  (y/n): " create_system_group
    if [[ $create_system_group == "y" || $create_system_group == "Y" ]]; then
        create_system_group
    else
        echo -e "\nℹ️  Skipping system group creation."
    fi
    ...
    read -p "➡️  (y/n): " create_ssh_keys
    if [[ $create_ssh_keys == "y" || $create_ssh_keys == "Y" ]]; then
        setup_ssh_keys
    else
        echo -e "\nℹ️  Skipping SSH key creation."
    fi
else
    echo -e "\nℹ️  Skipping Ansible user creation."
fi
